    ...
    class CustomImage extends ImageTool {
      removed() {
       ...
       const data = this._data.file.url
        fetch('http://localhost:3000/studies/StudyImageDelete',
        {method:'DELETE',
        headers:{
...
export var StudyTextEditorConfig = function(getid:number | undefined) {
    return{
    holder: 'editor-js',
    tools: { 
      header: {
      ...
      image: {
        ...
        config: {
          endpoints: {
            byFile: 'http://localhost:3000/studies/picload', // Your backend file uploader endpoint
          },
